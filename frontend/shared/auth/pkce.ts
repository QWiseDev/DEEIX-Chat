...
function sha256Fallback(input: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + sum1 + choose + (SHA256_ROUND_CONSTANTS[index] ?? 0) + (words[index] ?? 0)) >>> 0;
...
export async function createPKCEChallenge(verifier: string): Promise<string> {
  ...
  if (subtle) {
    try {
      return base64URL(new Uint8Array(await subtle.digest("SHA-256", input)));
    } catch {
      // 非安全上下文或受限浏览器中回退到纯 JavaScript SHA-256。
    }
  }
